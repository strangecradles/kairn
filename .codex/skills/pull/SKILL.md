...
8. Verify with project checks: `npm run build && npx tsc --noEmit && npm test`.
...
- For generated files (e.g., `dist/`), resolve source files first, then rerun
  `npm run build` to regenerate cleanly.
- For import conflicts where intent is unclear, accept both sides first, finish
  the merge, then run lint/type checks to remove unused or incorrect imports.
- After resolving, ensure no conflict markers remain:
  - `git diff --check`
- When unsure, note assumptions and ask for confirmation before finalizing.

## When To Ask The User (Keep To A Minimum)

Do not ask for input unless there is no safe, reversible alternative. Prefer a
best-effort decision, document the rationale, and proceed.
...
- The branch is not the intended target, or the remote/branch names cannot be
  determined locally.
